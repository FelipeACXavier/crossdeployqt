...
static std::optional<BinaryType> detectBinaryType(const fs::path& p, std::string& whyNot)
{
  ...
  auto u32le_at = [&](std::uint64_t off, std::uint32_t& out) -> bool {
    ...
    if (!f.read(reinterpret_cast<char*>(t), 4))
      return false;
  ...
  if (buf[0] == 'M' && buf[1] == 'Z')
  {
    ...
    if (fileSize >= 0x40 && u32le_at(0x3C, e_lfanew))
    {
      if (e_lfanew <= fileSize - 4)
      {
        std::uint8_t sig[4] = {0};
        f.clear();
        f.seekg(static_cast<std::streamoff>(e_lfanew), std::ios::beg);
  ...
  if (be == FAT_MAGIC || be == FAT_MAGIC_64 || be == FAT_CIGAM || be == FAT_CIGAM_64)
  {
    ...
    if (!ok)
    {
      whyNot = "truncated fat header";
      return std::nullopt;
    }

    if (nfat_arch == 0 || nfat_arch > 64)
    {  // typical range is small (1–5)
    ...
    if (need > fileSize)
    {
      whyNot = "fat header larger than file";
      return std::nullopt;
    }
...
static std::vector<std::string> computeMissingTools(BinaryType type, bool useQML)
{
  ...
  if (!haveQtpaths)
    missing.push_back(qtpathsBin.empty() ? std::string("qtpaths") : (qtpathsBin + " (from QTPATHS_BIN)"));

  // common tools
  if (useQML && !programOnPath("qmlimportscanner"))
    missing.push_back("qmlimportscanner");
  if (!programOnPath("lconvert"))
    missing.push_back("lconvert");

  // per-platform
  if (type == BinaryType::ELF)
  {
    if (!programOnPath("objdump"))
      missing.push_back("objdump");
    if (!programOnPath("patchelf"))
      missing.push_back("patchelf");
  }
  else if (type == BinaryType::PE)
  {
    if (!programOnPath("x86_64-w64-mingw32-objdump"))
      missing.push_back("x86_64-w64-mingw32-objdump");
  }
  else
  {  // Mach-O
    if (!programOnPath("llvm-otool"))
      missing.push_back("llvm-otool");
    if (!programOnPath("llvm-install-name-tool"))
      missing.push_back("llvm-install-name-tool");
  }
  return missing;
}
...
static QtPathsInfo queryQtPaths()
{
  ...
  if (code == 0)
    info.qtInstallLibs = fs::path(trim(libs));
  std::string bins = runCommand(qtpathsBin + " --query QT_INSTALL_BINS", code);
  if (code == 0)
    info.qtInstallBins = fs::path(trim(bins));
  std::string prefix = runCommand(qtpathsBin + " --query QT_INSTALL_PREFIX", code);
  if (code == 0)
    info.qtInstallPrefix = fs::path(trim(prefix));
  std::string plugins = runCommand(qtpathsBin + " --query QT_INSTALL_PLUGINS", code);
  if (code == 0)
    info.qtInstallPlugins = fs::path(trim(plugins));
  std::string qml = runCommand(qtpathsBin + " --query QT_INSTALL_QML", code);
  if (code == 0)
    info.qtInstallQml = fs::path(trim(qml));
  std::string trans = runCommand(qtpathsBin + " --query QT_INSTALL_TRANSLATIONS", code);
  if (code == 0)
    info.qtInstallTranslations = fs::path(trim(trans));
  // Validate that queried directories exist; otherwise, leave empty so we fall back to env paths
  std::error_code ec;
  if (!info.qtInstallQml.empty() && !fs::exists(info.qtInstallQml, ec))
    info.qtInstallQml.clear();
  if (!info.qtInstallPlugins.empty() && !fs::exists(info.qtInstallPlugins, ec))
    info.qtInstallPlugins.clear();
  if (!info.qtInstallTranslations.empty() && !fs::exists(info.qtInstallTranslations, ec))
    info.qtInstallTranslations.clear();
  return info;
}
...
static void ensureEnvForResolution(ResolveContext& ctx)
{
  ...
  else if (ctx.plan.type == BinaryType::PE)
  {
    ...
    for (const auto& p : pv)
      if (!p.empty())
        addSearchDir(ctx, p);
    if (!qtBins.empty())
      addSearchDir(ctx, qtBins);  // MinGW Qt DLLs live in bin
    // Prepend Qt bins to PATH
    if (!qtBins.empty())
    {
      std::string newPath = qtBins.string();
      if (!path.empty())
        newPath += std::string(1, pathListSep()) + path;
      setEnv("PATH", newPath);
    }
    ...
    for (const auto& p : pv)
    {
      if (p.size() > 4 && p.rfind("/bin", p.size() - 4) != std::string::npos)
      {
        ...
        fs::path q1 = base / "qml";
        if (fs::exists(q1, ec2))
          ctx.qmlImportPaths.push_back(q1);
        ec2.clear();
        fs::path q2 = base / "lib" / "qt-6" / "qml";
        if (fs::exists(q2, ec2))
          ctx.qmlImportPaths.push_back(q2);
      }
    }
  }
  else
  {
    ...
    for (const auto& p : splitPaths(dyld, pathListSep()))
      if (!p.empty())
        addSearchDir(ctx, p);
    std::string dyldfw = getEnv("DYLD_FRAMEWORK_PATH");
    for (const auto& p : splitPaths(dyldfw, pathListSep()))
      if (!p.empty())
        addSearchDir(ctx, p);
    if (!qtLibs.empty())
      addSearchDir(ctx, qtLibs);
    ...
    if (!qtLibs.empty())
    {
      ...
      if (!dyld.empty())
        newDyld += std::string(1, pathListSep()) + dyld;
      setEnv("DYLD_LIBRARY_PATH", newDyld);
      std::string newDyldFw = qtLibs.string();
      if (!dyldfw.empty())
        newDyldFw += std::string(1, pathListSep()) + dyldfw;
  ...
  {
    std::error_code ec;
    if (fs::exists(ctx.qt.qtInstallQml, ec))
      ctx.qmlImportPaths.push_back(ctx.qt.qtInstallQml);
  }
  ...
  for (const auto& p : splitPaths(qml2Env, pathListSep()))
  {
    ...
    std::error_code ec;
    if (fs::exists(p, ec))
      ctx.qmlImportPaths.emplace_back(p);
  }
  ...
  for (const auto& p : splitPaths(envRoots, pathListSep()))
    if (!p.empty())
      ctx.cliQmlRoots.emplace_back(p);
}
...
static fs::path expandMachOToken(const std::string& p, const fs::path& subjectBin, const fs::path& mainExe)
{
  ...
  if (p.rfind("@loader_path/", 0) == 0)
    return fs::weakly_canonical(dir / p.substr(13));
  if (p.rfind("@executable_path/", 0) == 0)
    return fs::weakly_canonical(mainExe.parent_path() / p.substr(17));
  return fs::path(p);
}
                                             ...
                                             const ResolveContext& ctx)
{
  ...
  for (const auto& rp : subjectRpaths)
  {
    ...
    std::error_code ec2;
    if (fs::exists(cand, ec2))
      return fs::weakly_canonical(cand, ec2);
  }
...
static bool shouldDeployLibrary(const fs::path& libPath, const std::string& sonameOrDll, BinaryType type, const ResolveContext& ctx)
{
  ...
  auto inQtPath = [&]() -> bool {
    if (!ctx.qt.qtInstallLibs.empty() && libPath.string().find(ctx.qt.qtInstallLibs.string()) == 0)
      return true;
    if (!ctx.qt.qtInstallBins.empty() && libPath.string().find(ctx.qt.qtInstallBins.string()) == 0)
      return true;
    if (!ctx.qt.qtInstallPrefix.empty() && libPath.string().find(ctx.qt.qtInstallPrefix.string()) == 0)
      return true;
    return false;
  };
  if (type == BinaryType::ELF)
  ...
  {  // Mach-O
    ...
    if (s.rfind("/System/Library/Frameworks/", 0) == 0 || s.rfind("/usr/lib/", 0) == 0)
      return false;
    return isQtLibraryName(base) || inQtPath() || dir == ctx.plan.binaryPath.parent_path();
  }
}
...
static const std::vector<std::string>& machoRpathsFor(const fs::path& subject, ParseCache& cache)
{
  ...
  if (it != cache.machoRpathsByPath.end())
    return it->second;
...
static ParseResult parseMachO(const fs::path& bin)
{
  ...
  while (std::getline(iss, line))
  {
    if (first)
    {
      first = false;
      continue;
    }  // skip header line containing the binary path
...
static void applyOverlays(const DeployPlan& plan)
{
  for (const auto& ov : plan.overlays)
  {
    if (ov.empty())
      continue;
    std::error_code ec;
    if (!fs::exists(ov, ec) || !fs::is_directory(ov, ec))
      continue;
    if (isVerbose())
      std::cout << "[overlay] merge " << ov << " -> " << plan.outputRoot << "\n";
...
// --- Windows (PE) Qt Core prefix/infix binary patching ---
// Mirrors windeployqt behavior: overwrite built-in qt_prfxpath/qt_epfxpath/qt_hpfxpath
// inside Qt6Core.dll with a short relative value (".") and NUL-pad to original length.
static bool patchQtCoreDllPrefixInfixPE(const fs::path& qtCorePath)
{
  std::error_code ec;
  if (!fs::exists(qtCorePath, ec) || !fs::is_regular_file(qtCorePath, ec))
    return false;
  ...
  auto patchAsciiKey = [&](const std::string& keyWithEq, const std::string& replacement) -> bool {
    ...
    while (true)
    {
      pos = std::search(buf.begin() + static_cast<std::ptrdiff_t>(pos), buf.end(), keyWithEq.begin(), keyWithEq.end()) - buf.begin();
      if (pos == buf.size())
        break;
      ...
      if (valLen >= replacement.size())
      {
        ...
        {
          if (buf[valStart + i] != static_cast<std::uint8_t>(replacement[i]))
          {
            needChange = true;
            break;
          }
        ...
        if (needChange)
        {
          // Write replacement and pad with NULs to preserve size
  ...
  auto patchUtf16Key = [&](const std::u16string& keyWithEq, const std::u16string& replacement) -> bool {
    ...
    while (true)
    {
      ...
      size_t scan = valStart;
      // UTF-16LE terminator is two zero bytes (0x00 0x00), but we can stop on the first pair
      ...
      const size_t valLen = scan - valStart;  // in bytes
      if (valLen >= repBytes.size())
      {
        bool needChange = false;
        for (size_t i = 0; i < repBytes.size(); ++i)
        {
          if (buf[valStart + i] != repBytes[i])
          {
            needChange = true;
            break;
          }
...
static void copyPluginsPE(const ResolveContext& ctx, const DeployPlan& plan, const std::vector<fs::path>& resolvedLibs)
{
  ...
  if (!ctx.qt.qtInstallPlugins.empty())
    pluginRoots.push_back(ctx.qt.qtInstallPlugins);
  // From env MINGW_QT_PLUGINS (colon-separated)
  std::string mingwPlugins = getEnv("MINGW_QT_PLUGINS");
  for (const auto& p : splitPaths(mingwPlugins, pathListSep()))
    if (!p.empty())
      pluginRoots.emplace_back(p);
  ...
  for (const auto& p : splitPaths(path, pathListSep()))
  {
    if (p.size() > 4 && p.rfind("/bin", p.size() - 4) != std::string::npos)
    {
      ...
      if (fs::exists(root1, ec))
        pluginRoots.push_back(root1);
      ec.clear();
      fs::path root2 = base / "lib" / "qt-6" / "plugins";
      if (fs::exists(root2, ec))
        pluginRoots.push_back(root2);
    }
  }
  ...
  for (const auto& src : pluginRoots)
  {
    ...
    for (const char* name : {"qjpeg.dll", "qico.dll", "qgif.dll", "qpng.dll"})
    {
      ...
      if (fs::exists(p))
        copyFileOverwrite(p, plan.outputRoot / "plugins" / "imageformats" / p.filename());
    }
    break;
  }
}
...
static void copyPluginsELF(const ResolveContext& ctx, const DeployPlan& plan)
{
  ...
  if (fs::exists(platformSo))
    copyFileOverwrite(platformSo, plan.outputRoot / "usr" / "plugins" / "platforms" / platformSo.filename());
  // imageformats
  for (const char* name : {"libqjpeg.so", "libqico.so", "libqgif.so", "libqpng.so"})
  {
    fs::path p = src / "imageformats" / name;
    if (fs::exists(p))
      copyFileOverwrite(p, plan.outputRoot / "usr" / "plugins" / "imageformats" / p.filename());
  }
  ...
  for (const auto& lib : libs)
  {
    ...
    if (!frameworkRoot.empty())
    {
      ...
      if (!copiedFrameworks.count(key))
      {
        ...
        if (isVerbose())
          std::cout << "[macho-copy] framework: " << frameworkRoot << " -> " << dst << "\n";
    ...
    else
    {
      ...
      if (isVerbose())
        std::cout << "[macho-copy] dylib: " << lib << " -> " << dest << "\n";
      if (!copyFileOverwrite(lib, dest))
      {
        std::cerr << "Warning: failed to copy " << lib << " -> " << dest << "\n";
      }
...
static void copyPluginsMachO(const ResolveContext& ctx, const DeployPlan& plan)
{
  ...
  if (fs::exists(cocoa))
    copyFileOverwrite(cocoa, dstBase / "platforms" / cocoa.filename());
  // imageformats
  for (const char* name : {"libqjpeg.dylib", "libqico.dylib", "libqgif.dylib", "libqpng.dylib"})
  {
    fs::path p = src / "imageformats" / name;
    if (fs::exists(p))
      copyFileOverwrite(p, dstBase / "imageformats" / p.filename());
  }
...
static std::vector<fs::path> discoverQmlRoots(const ResolveContext& ctx)
{
  ...
  {
    if (!cwd.empty() && hasQml(cwd))
      roots.push_back(cwd);
    fs::path binDir = ctx.plan.binaryPath.parent_path();
    if (!binDir.empty() && hasQml(binDir))
      roots.push_back(binDir);
  }
  // Deduplicate
...
static void copyQmlModules(const ResolveContext& ctx, const DeployPlan& plan)
{
  ...
  {
    if (isVerbose())
      std::cout << "[qml] module: " << m.sourcePath << " -> " << (qmlDestBase / m.relativePath) << "\n";
           ...
           it != fs::recursive_directory_iterator(); ++it)
      {
        ...
        if (plan.type == BinaryType::MACHO)
        {
          ...
          if (target.extension() == ".dylib")
          {
            ...
            if (isVerbose())
              std::cout << "[qml] stage dylib: " << target << " -> " << moved << "\n";
    ...
    for (const auto& dep : prChild.dependencies)
    {
      ...
      if (found)
      {
        ...
        if (shouldDeployLibrary(*found, dep, plan.type, ctx))
        {
          ...
          if (isVerbose())
            std::cout << "[qml-deps]     push: " << *found << "\n";
          if (!visited.count(fkey))
            stack.push_back(*found);
          result.insert(fkey);
        }
      }
    ...
    if (posFramework != std::string::npos)
    {
      ...
      if (posVersions != std::string::npos)
      {
        ...
        if (slash != std::string::npos)
          version = afterVersions.substr(0, slash);
      }
...
static void fixInstallNamesMachO(const DeployPlan& plan)
{
  ...
  auto findFrameworkBinary = [&](const fs::path& frameworkRoot) -> std::optional<fs::path> {
    ...
    if (fs::exists(versions, lec) && fs::is_directory(versions, lec))
    {
      ...
      if (fs::exists(current, lec))
      {
        ...
        if (fs::exists(cand, lec) && fs::is_regular_file(cand, lec))
          return cand;
      }
  ...
  if (fs::exists(fwDir, ec))
  {
         ...
         it != fs::recursive_directory_iterator(); ++it)
    {
      if (!it->is_directory(ec))
        continue;
      if (it->path().extension() != ".framework")
        continue;
...
static std::vector<std::string> detectLanguagesFromEnv()
{
  ...
  if (std::find(langs.begin(), langs.end(), std::string("en")) == langs.end())
    langs.push_back("en");
  return langs;
}
...
{
  if (!plan.languages.empty())
    return plan.languages;
...
{
  if (plan.type == BinaryType::MACHO)
    return plan.outputRoot / "Contents" / "Resources" / "translations";
  if (plan.type == BinaryType::ELF)
    return plan.outputRoot / "usr" / "translations";
...
static std::vector<fs::path> listModuleCatalogsForLang(const fs::path& qtTransDir, const std::string& lang)
{
  ...
  std::error_code ec;
  if (!fs::exists(qtTransDir, ec) || !fs::is_directory(qtTransDir, ec))
    return files;
...
static void resolveDependenciesPE(const DeployPlan& plan)
{
  ...
  for (const auto& p : libs)
  {
    ...
    if (lower == "qt6core.dll")
    {
      ...
      {
        // If not at root, try alongside copied location
        staged = plan.outputRoot / p.filename();
      }
      if (fs::exists(staged, ec))
      {
        if (isVerbose())
          std::cout << "[pe] patch Qt6Core.dll: " << staged << "\n";
...
int main(int argc, char** argv)
{
  try
  {
    ...
    DeployPlan plan{*maybeType, args.binaryPath, normalizedOut, args.qmlRoots, args.languages, args.overlays, args.useQML};

    // Verify external tool availability for this platform
    {
      std::vector<std::string> missing = computeMissingTools(plan.type, plan.useQML);
      if (!missing.empty())
      {
    ...
    {
      case BinaryType::PE:
        resolveDependenciesPE(plan);
        break;
      case BinaryType::ELF:
        resolveDependenciesELF(plan);
        break;
      case BinaryType::MACHO:
        resolveDependenciesMachO(plan);
        break;
    }
